...
def upload_files():
    ...
    for file in files:
        ...
        if file and file.filename.lower().endswith('.pdf'):
            ...
            file_path = os.path.join(upload_dir, f"{unique_id}_{original_filename}")
            file.save(file_path)
            text = extract_text_from_pdf(file_path)
            pdf_text_storage[unique_id] = text
            all_texts.append(text)
            file_info.append({"file_name": original_filename, "file_id": unique_id})
        else:
    ...
    return jsonify({"message": "Files uploaded and text extracted", "file_info": file_info}), 200
...
def cancel_file(file_id):
    if file_id in pdf_text_storage:
        ...
        file_name = [fname for fname in os.listdir('uploads') if fname.startswith(file_id)]
        if file_name:
            file_path = os.path.join('uploads', file_name[0])
